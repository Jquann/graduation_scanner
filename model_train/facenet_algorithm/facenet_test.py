...
def load_facenet_model():
    try:
        print("Building FaceNet-like model...")
        # Use InceptionV3 as backbone (similar to FaceNet architecture)
        ...
        model = KerasModel(inputs=base_model.input, outputs=embeddings)
        print("FaceNet-like model loaded successfully!")
        return model
...
def get_facenet_embedding(img_path):
    try:
        if not os.path.exists(img_path) or facenet_model is None:
            return None
        ...
        warnings.warn(f"FaceNet embedding error for {img_path}: {str(e)}")
        return None
...
print(f"Testing FaceNet with {len(pairs)} pairs...")
...
if y_true:
    ...
    bars = plt.bar(metrics, values, color='lightgreen', alpha=0.7)
    plt.title(f'FaceNet Performance on LFW Dataset ({len(pairs)} pairs)')
    plt.ylabel('Score')
    plt.ylim(0, 1)
    ...
    filename = f'facenet_results_{timestamp}.png'
